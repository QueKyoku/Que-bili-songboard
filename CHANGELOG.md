...
- **控制台会告诉你有没有新版本。** 启动时查一次 GitHub 上的最新 tag，
  比当前版本新就在控制台顶部加一条提醒（带「看更新日志」和「知道了」），
  连接状态那儿也一直显示当前版本号。

  为什么要有：主播下的是 zip，没人会隔几天回仓库看有没有新版；而这个项目的坑
  （cookie 过期、房间短号收不到弹幕、脚本乱码）大多是别人先撞上才修的。

  - **查更新绝不影响启动**：断网、超时、被 GitHub 限流一律静默当「不知道」，
    不报错、不重试、不卡住。而且它是独立后台任务 —— `urlopen` 是同步阻塞的，
    直接塞进事件循环的话断网会让整个点歌板僵住 5 秒（连弹幕都不处理）。
  - **查不到时控制台什么都不显示**，不会假装「已是最新」。服务端用
    `checked` 区分「不知道」和「已是最新」，两者显示完全不同。
  - **版本比较按数字逐段比，不是字符串比**：`'0.10.0' < '0.9.9'` 在字符串
    比较下是成立的，那样发了 0.10.0 就没人收得到提示 —— 而且只有版本号跨到
    两位数才复现，几乎不可能被发现。自检里有专门一条盯着它。
  - 比的是 **git tag**，不是 GitHub Releases —— 本项目的发版方式就是打 tag，
    Releases 页面一直是空的。
  - 默认开着，`update_check.enabled` 可关（关掉后一次请求都不发）；
    `interval_hours` 默认 6 小时（一场直播动辄几小时，只在启动查一次会漏掉
    播到一半才发的版）。
...
- **`App()` 在「这个线程跑过 `asyncio.run()`」之后会直接崩**：
  `RuntimeError: There is no current event loop in thread 'MainThread'`。
  原因是 Python 3.12 的 `asyncio.get_event_loop()` 在 current loop 被清空后会抛错，
  而 `asyncio.run()` 结束时正好会清。改成用 `get_running_loop()` 取运行中的循环、
  没有就自己起一个；`run()` 里再对齐一次 —— 否则 `self.loop.create_task()`
  会把任务挂到另一个循环上，表现是「任务静默不跑」，极难查。
- **日志在终端里会延迟到下一次按键才显示**：`self.log()` 的 `print` 没有 `flush`，
  stdout 接管道/重定向时是块缓冲。实测「版本检查」这行要等主播按一次回车才出现，
  看起来就像这条检查根本没跑（控制台网页是实时的，所以只有终端能看到这个现象）。
  加 `flush=True`。
...
- `selftest.py` 加了 `test_version_check`（41 项）：版本号解析与比较
  （含 `0.10.0 > 0.9.9` 这条字符串比较会判反的）、断网时静默降级、
  关掉开关后一次请求都不发、控制台元素齐全且 JS 引用的 id 都存在，
  以及上面那条「跑过 `asyncio.run()` 之后还能构造 App」的回归。
