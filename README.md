...
当前版本 **v0.4.0** · 更新日志见 [`CHANGELOG.md`](CHANGELOG.md) ·
...
**手机上装网易云音乐 App 的话用这个 —— 连命令行都不用碰。**
项目根目录里有个 `扫码登录.pyw`，**双击它**，会弹出一个窗口：
...
- **双击就能跑**（`.pyw` 后缀不会弹出黑色控制台窗口）
- **没装 `qrcode` 也没关系** —— 界面上会多出一个「自动安装 qrcode」按钮，
  点一下自动装好并继续，全程不用敲命令
- 扫完没及时确认、或者二维码过期了，点「重新生成二维码」再来一次
- 想做成一个 **exe** 发给别人：跑一次 `tools\build_gui.ps1`，
  把产物放到项目根目录再双击（详见第十二章的项目结构）
...
diag_room.py               弹幕直连诊断
扫码登录.pyw              [双击即用] 图形界面扫码登录（给不想碰命令行的人）
