...
```jsonc
{
  ...
  "update_check": {
    "enabled": true              // 启动时查有没有新版本，控制台顶上提醒
  }
}
```
...
启动时会去 GitHub 查一次最新版本，比你正在跑的新就在**控制台顶部**加一条提醒
（带「看更新日志」链接和一个「知道了」按钮）。连接状态那儿也一直显示当前版本号，
所以随手就能看出自己跑的是哪一版。

为什么要有：你下的是 zip，没人会隔几天回仓库看有没有新版；而这个项目踩过的坑
（cookie 过期、房间短号收不到弹幕、脚本乱码）大多是别人先撞上才修的 ——
提醒一句能省掉一堆「我这是不是坏了」。

```jsonc
"update_check": {
  "enabled": true,        // 不想要就 false，关掉后一次网络请求都不发
  "interval_hours": 6,    // 隔多久再查一次。0 = 只在启动时查一次
...
- **它永远不会影响启动。** 断网、超时、GitHub 被限流，一律静默当成「不知道」——
  不报错、不重试、不卡住。查不到时控制台**什么都不显示**，
  而不是假装「已是最新」（那是在骗你）。
- **它只发一个请求、只收一个版本号。** 就是读一下 GitHub 的 tag 列表，
  不发送 cookie、房间号或任何本地信息。
- 比的是 **git tag**，不是 GitHub Releases —— 本项目的发版方式就是打 tag 推上去，
  Releases 页面一直是空的。
...
**手机上装网易云音乐 App 的话用这个 —— 连命令都不用敲。**
项目根目录里有个 **`扫码登录.bat`，双击它**，会弹出一个窗口：
...
用手机上的**网易云音乐 App**（不是微信扫一扫）扫一下、在手机上点确认，
窗口里就会变成：
...
cookie 这时候已经写进 `config.json` 了，**不用重启程序**。

这个 `.bat` 干的事就是**把所有前置条件都检查一遍、缺什么补什么**：

| 情况 | 它会怎么办 |
| --- | --- |
| 没装 Python | 明确告诉你怎么装（含「务必勾选 Add python.exe to PATH」），并给出另一条路：直接用打包好的 exe |
| Python 版本太旧（低于 3.10） | 报出当前版本，让你去升级 |
| 没有 `tkinter`（精简版 Python 常见） | 提示换官方安装包，或改用命令行版扫码 |
| 缺 `qrcode`（画二维码用的） | **自动 `pip install qrcode`**，装不上就给手动命令 |
| 一切正常 | 用 `pythonw` 拉起图形界面，不留黑色控制台窗口 |

> ⚠️ **为什么检测写在 `.bat` 而不是 `.pyw` 里？**
> 因为没装 Python 的时候，`.pyw` 双击**根本不会被执行**
> （Windows 只会弹「你要用什么打开这个文件？」）——
> 那样的话里面的检测代码一行都跑不到。
...
- 想做成一个 **exe** 发给**没装 Python**的人：跑一次 `tools\build_gui.ps1`，
...
songboard/                 主程序（Python）
  ...
  qrlogin.py               扫码登录的逻辑（命令行版和图形界面版共用）
  version_check.py         ★ 查有没有新版本（控制台顶部提醒用）
