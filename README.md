...
当前版本 **v0.2.6** · 更新日志见 [`CHANGELOG.md`](CHANGELOG.md) ·
...
**cookie 只干两件事**：把歌名搜成歌曲 id、查歌时长。它**不**用来写歌单
（程序一个字节都不写你的歌单）。但没有它，歌就插不进播放队列 ——
因为压根拿不到歌曲 id。

那能不能不用 cookie？**不能，别折腾了**（实测过）：

- 项目用的是网易云网页端登录态的接口。不带 cookie 时它返回
  `code=50000005`（未登录），搜不到任何东西
- 网易云确实还有个匿名的老接口能搜到歌，但它**会限流**：
  连打 40 来次就全部返回 `code=406`，等半分钟才恢复。
  直播场景下随时可能踩上，不能依赖

怎么确认填的 cookie 有效：**保存后看控制台那张卡片**。它会当场验证并显示

| 显示 | 意思 |
| --- | --- |
| `已登录：你的昵称` | 有效，可以搜歌 |
| `cookie 没能验证通过` | 填了但网易云没认（多半是复制漏了 / 过期了），重新复制一份 |
| `没填 cookie，搜不了歌` | 没填 |
| `未启用（搜不了歌，歌插不进队列）` | 上面的「启用」没勾 |

程序启动时也会自己验一次，日志里会写「网易云搜索：搜索可用，已登录：xxx」。

> ⚠️ cookie 会过期（`MUSIC_U` 有有效期，通常几个月）。过期后**不会再报
> "搜不到这首歌"那种误导人的错**，而是明确写「网易云登录态无效
> （code=50000005），cookie 可能已过期」—— 看到这句就是该换 cookie 了。
